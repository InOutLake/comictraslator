...
from gateway.src.apps.documents.services.services import (
    DocumentFactory,
...
class DocumentsUseCaseProtocol(Protocol):
    async def list_documents(
        self: Self, doc_data: List[DBDocumentSchema]
    ) -> list[Document]: ...
...
async def get_documents_use_case(
    document_factory: DocumentFactory,
) -> DocumentsUseCaseProtocol:
